...
template sudoku() {
    ...
    component rng_puzzle[9][9];
    component rng_solution[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rng_puzzle[i][j] = RangeProof(32);
            rng_puzzle[i][j].range[0] <== 0;
            rng_puzzle[i][j].range[1] <== 9;
            rng_puzzle[i][j].in <== puzzle[i][j];
            assert(rng_puzzle[i][j].out == 1); //[assignment] change assert() to use your created RangeProof instead

            rng_solution[i][j] = RangeProof(32);
            rng_solution[i][j].range[0] <== 0;
            rng_solution[i][j].range[1] <== 9;
            rng_solution[i][j].in <== solution[i][j];
            assert(rng_solution[i][j].out == 1); //[assignment] change assert() to use your created RangeProof instead
